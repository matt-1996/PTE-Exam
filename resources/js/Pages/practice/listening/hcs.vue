<template>
    <MainLayout>
        <v-container>
        <Drawer :datas="Hcss" :links="links" title="Highlight Correct Summary" routeName="practice.l_hcs.show" :image="image" />
        <v-row class="mt-2" justify="center" align="center" offset-md="1">
                ...
                <v-col cols="9" class="">
                    <h2 class="text-2xl my-2">Highlight Correct Summary
                        ...
                        <div v-if="showBookmarkList" @mouseover="showBookmarkList = true" class="mr-32 w-32 shadow-2xl drop-shadow-xl">
                            <div class="p-2 absolute top-0 right-2 z-10 bg-white" >
                                <div class="flex py-2 hover:bg-black cursor-pointer" v-for="bookamrk in bookamrks">
                                    ...
                                    <div>
                                        <p class="text-gray-500" @click="bookmark(hcs.id,bookamrk.color)">{{ bookamrk.title }}</p>
                                    </div>
                                </div>
                            </div>
                    </div>
            ...
            <v-row>
                <vue-countdown v-if="prepare" :time=" 1 * hcs.duration * 1000" v-slot="{ minutes,seconds }" @end="onPrepareEnd(hcs.duration)">
                    <div class="text-red-400">Prepare: 0{{ minutes }}:{{ seconds }}</div>
                </vue-countdown>

                <vue-countdown v-if="timer" :time=" 1 * hcs.duration * 1000" v-slot="{ minutes,seconds }" @end="onTimeEnd">
            ...
            <v-row>
                ...
                <v-col cols="auto">
                    <div v-if="previousPracticeId !== hcs.id ?? hcs.id">
                        <Link :href="route('practice.l_hcs.show' , previousPracticeId)">
                            <v-btn >
                                Previous
                            </v-btn>
                        </Link>
                ...
                <v-col cols="auto">
                    <div v-if="nextPracticeId !== 0">
                        <Link :href="route('practice.l_hcs.show' , nextPracticeId ?? hcs.id)">
...
const image = ref(publicPath.value + '/images/hcs_l_v2.png')
...
const practiceArray = reactive([])
    ...
    const currentPage = ref(props.hcs.id)

function getHcssIndex(){
        axios.get(route('practice.l_hcs.index')).then(function(res){
            Hcss.value = res.data.message
            drawer.value = true
            links.value = res.data.message.links
            nextPracticeId.value = nextPractice(currentPage, practiceArray, Hcss)
            previousPracticeId.value = previousPractice(currentPage, practiceArray , Hcss)
        })
    }
